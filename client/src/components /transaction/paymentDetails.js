...
function PaymentDetails({ open }) {
  ...
  const { payment_status, transfer, recepient, payment_timestamp } =
    useSelector((state) => state.transfer_details);
  ...
  const usdt = (resolve, reject) =>
    resolve(
      getCryptoPaymentBEP20("0x337610d27c682E347C9cD60BD4b3b107C9d34dDd")
    );
  const eth = (resolve, reject) =>
    resolve(
      getCryptoPaymentBEP20("0xd66c6B4F0be8CE5b39D52E0Fd1344c389929B378")
    );
  const busd = (resolve, reject) =>
    resolve(
      getCryptoPaymentBEP20("0xed24fc36d5ee211ea25a80239fb8c4cfd80f12ee")
    );
  const btc = (resolve, reject) =>
    resolve(
      getCryptoPaymentBEP20("0x6ce8da28e2f864420840cf74474eff5fd80e65b8")
    );
  const bnb = (resolve, reject) => resolve(getCryptoPayment());

  const checkPayment = async () => {
    Promise.all([
      transfer.tokenValue === "USDT"
        ? new Promise(usdt)
        : transfer.tokenValue === "ETH"
        ? new Promise(eth)
        : transfer.tokenValue === "BUSD"
        ? new Promise(busd)
        : transfer.tokenValue === "BNB"
        ? new Promise(bnb)
        : transfer.tokenValue === "BTC"
        ? new Promise(btc)
        : "",
    ]).then((res) => {
      ...
      if (
        !(response < payment_timestamp.timestamp) &&
        !(response > payment_timestamp.expriryTimestamp)
      ) {
        const sentToken = res[0].result[0].value / 1000000000000000000;
        const paidAmount =
          (res[0].result[0].value / 1000000000000000000) * transfer.rate;
        setSuccess(true);
        ...
        <Modal.Body bsPrefix="modal-body">
          <>
            <div className="modal-heading">
              <p>
                {" "}
                <BiStopwatch size={20} style={{ fill: "white" }} /> Time
                remaining for your payment
              </p>
              <CountdownTimer />
            </div>
            ...
            <div className="address-details">
              ...
              <RiFileCopyLine
                ...
                onClick={() =>
                  navigator.clipboard.writeText(
                    `${transfer.sendAmount} ${transfer.tokenValue}`,
                    toastNotification("Copied to Clipboard")
                  )
                }
              />
            ...
            <div className="cancel" onClick={handleClose}>
              <p>
                <MdOutlineClose size={20} style={{ fill: "red" }} /> Cancel
                Payment transaction
              </p>
            </div>
